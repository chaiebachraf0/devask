...
export class AddcommfComponent implements OnInit {
  ...
  myDate = new Date();
  constructor(private route: ActivatedRoute, private router: Router, private toastr: ToastrService, private commandeclService: commandeclService, private productService: ProductService, private four: FournisseursService, private paiement: PaiementService) { }
  ngOnInit(): void {
 ...
 */
  let listAchat: Array<ListProduct> = new Array();
    ...
    this.commandeclService.insertDataf(this.facture).subscribe(res => {
      this.router.navigate(['rachat/achat/commf']);
      this.toastr.success('', 'Commande Enregistrée');
    });
